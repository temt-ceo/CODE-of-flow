...
window.createPlayer = async function (playerName) {
  var transactionId = await fcl.mutate({
    cadence: `
      ...
      import CodeOfFlowBeta5 from 0x9e447fb949c3f1b6
      ...
      transaction(nickname: String) {
        prepare(signer: AuthAccount) {
          ...
          signer.save(<- CodeOfFlowBeta5.createPlayer(nickname: nickname, flow_vault_receiver: FlowTokenReceiver), to: CodeOfFlowBeta5.PlayerStoragePath)
          signer.link<&CodeOfFlowBeta5.Player{CodeOfFlowBeta5.IPlayerPublic}>(CodeOfFlowBeta5.PlayerPublicPath, target: CodeOfFlowBeta5.PlayerStoragePath)
          }
        execute {
          log("success")
        }
    ...
    cadence: `
      ...
      transaction() {
        prepare(signer: AuthAccount) {
          ...
          let player = signer.borrow<&CodeOfFlowBeta5.Player>(from: CodeOfFlowBeta5.PlayerStoragePath)
...
window.isRegistered = async function (address) {
  const result = await fcl.query({
    ...
    pub fun main(address: Address): &CodeOfFlowBeta5.Player{CodeOfFlowBeta5.IPlayerPublic}? {
        ...
        return account.getCapability<&CodeOfFlowBeta5.Player{CodeOfFlowBeta5.IPlayerPublic}>(CodeOfFlowBeta5.PlayerPublicPath).borrow()
    }
    `,
    args: (arg, t) => [
...
window.getCurrentStatus = async function (address) {
  const result = await fcl.query({
    ...
    pub fun main(address: Address): AnyStruct {
        let account = getAccount(address)
        let cap = account.getCapability<&CodeOfFlowBeta5.Player{CodeOfFlowBeta5.IPlayerPublic}>(CodeOfFlowBeta5.PlayerPublicPath).borrow()
...
window.getStarterDeck = async function () {
  const result = await fcl.query({
    ...
    import CodeOfFlowBeta5 from 0x9e447fb949c3f1b6
    pub fun main(): [UInt16] {
        return CodeOfFlowBeta5.getStarterDeck()
    }
    `,
    args: (arg, t) => [
    ]
...
window.getBalance = async function (address, playerId) {
  const result = await fcl.query({
    ...
    pub fun main(address: Address, player_id: UInt?): [CodeOfFlowBeta5.CyberScoreStruct] {
        let account = getAccount(address)
        let vaultRef = account.getCapability(/public/flowTokenBalance).borrow<&FlowToken.Vault{FungibleToken.Balance}>()
            ?? panic("Could not borrow Balance reference to the Vault")

        var retArr: [CodeOfFlowBeta5.CyberScoreStruct] = []
        if player_id != nil {
        ...
        let guestData = CodeOfFlowBeta5.CyberScoreStruct(player_name: "Guest")
